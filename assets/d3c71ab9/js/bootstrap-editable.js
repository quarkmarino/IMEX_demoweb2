...
(function ($) {
    ...
    $.fn.editable.defaults = {
        ...
        formTemplate:'<form class="form-inline" autocomplete="off">' + 
                       ...
                       '&nbsp;<button type="submit" class="btn btn-primary"><i class="icon-ok icon-white"></i></button>&nbsp;<button type="button" class="btn editable-cancel" id="disableEditor"><i class="icon-ban-circle"></i></button>' + 
            ...
            renderInput:function () {
                ...
                this.settings.onSourceReady.call(this, function () {
                        if (typeof this.settings.source === 'object' && this.settings.source != null) {
                            ...
                            var sortable = [];
                            for (var id in this.settings.source)
                                sortable.push([id, this.settings.source[id]])
                            sortable.sort(function(a, b) {
                                var textA = a[1].toUpperCase();
                                var textB = b[1].toUpperCase();
                                return (textA < textB) ? -1 : (textA > textB) ? 1 : 0;
                            });                            
                            $.each(sortable, $.proxy(function (key, value) {
                                this.$input.append($('<option>', { value:value[0] }).text(value[1]));
                            }, this));
